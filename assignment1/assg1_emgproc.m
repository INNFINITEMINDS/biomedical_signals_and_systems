...
emg2 = decimate(emg,4); %orignal Fs is 2000, reduce by 4 to Fs of 500
N2 = length(emg2); % data length
t2 = [1:N2]/Fs; % time axis
figure
plot(t2,emg2)
xlabel('Time (sec)')
ylabel('Amplitude (mV)')
title('Downsampled EMG Waveform')
set(gca,'xlim',[0, max(t2)]) % set the limits of time in the plot
%% Calculate periodogram of downsampled signal
x = detrend(emg2); % detrending
nfft2 = 2^nextpow2(N2); % number of FFT points
[P_per, f] = periodogram(x,[],nfft2,Fs); % calculate periodogram
...
ylabel('PSD (dB)')
title('Welch Method')
set(gca,'xlim',[0, Fs/2],'ylim',[-90 -20]) % set the limits of frequency and PSD in the plot
...
set(gca,'xlim',[0, Fs/2],'ylim',[-75 -20]) % set the limits of frequency and PSD in the plot
...
%plot(f, h)
xlabel('Frequency (Hz) logscale'); ylabel('X(f)(dB)');
title('Butterworth');
set(gca,'xlim',[-1000, 1000]) % set the limits of time in the plot
set(gca,'ylim',[-150, 25]) % set the limits of time in the plot
...
set(gca,'xlim',[0, Fs/2],'ylim',[-130 -20]) % set the limits of frequency and PSD in the plot
%% Calculate PSD of filtfilt signal using Welch Method
nfft = 2^nextpow2(N);
[PS,F] = pwelch(detrend(y2),hamming(2000),[],nfft, Fs);%default is hamming window with 50% overlap, change segment length 2000,1000, 5000
figure
plot(F,10*log10(PS))
xlabel('Frequency (Hz)')
ylabel('PSD (dB)')
...
nfft = 2^nextpow2(N);
[PS0,F0] = pwelch(detrend(emg),hamming(2000),[],nfft, Fs);%default is hamming window with 50% overlap, original signal
[PS1,F1] = pwelch(detrend(y2),hamming(2000),[],nfft, Fs);%default is hamming window with 50% overlap, filtered signal
...
set(gca,'xlim',[0, Fs/2],'ylim',[-110 -20]) % set the limits of frequency and PSD in the plot
